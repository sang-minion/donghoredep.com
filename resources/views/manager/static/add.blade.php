...
@section('content')
    <form action="" class="form-horizontal frmAdd" method="POST" id="frmAdd" name="frmAdd" enctype="multipart/form-data">
        ...
        <div class="form-group{{isset($errors)&& $errors->has('static_keyword') ? ' has-error' : '' }}">
            ...
            <div class="col-md-6 col-sm-9">
                <input id="static_keyword" type="text" class="form-control" name="static_keyword"
                       value="{{isset($data['static_title'])?$data['static_keyword']: old('static_keyword') }}" required
                       autofocus>
                @if (isset($errors)&&$errors->has('static_keyword'))
                    <span class="help-block">
                                        <strong>{{ $errors->first('static_keyword') }}</strong>
                                    </span>
                @endif
            </div>
        </div>
